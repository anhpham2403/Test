...
class CalculatorViewModel(context: Context, isCurrency1: Boolean,
    currency1: Currency) : BaseObservable() {
  companion object {
  ...
  fun commaButtonClick() {
    if (!value.contains(".")) {
      value += if (value.endsWith("+") || value.endsWith("-") || value.endsWith(
          "÷") || value.endsWith("×") || value.isEmpty()) {
        "0."
      } else {
        "."
      }
